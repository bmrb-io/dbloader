...
2. **The publish step has no transaction, and the commented-out attempt at one
   would not have helped.** Each table is `truncate table only` + `\copy`, so
   the truncate commits first: a failed copy leaves that table empty on the
   live server, and readers can see a partly reloaded database throughout.

   `fromcsv` carries a commented-out `-c begin ... -c truncate ... -c \copy
   ...
   `\copy` fails it carries on to the `-c commit` and commits the truncate
   anyway. Measured, on the same failing copy:

   | form | table after a failed copy |
   |---|---|
   | current (no transaction) | **empty** |
   ...
   was abandoned. Locking is not the explanation for *that* snippet: it is
   per-table, so `TRUNCATE`'s `ACCESS EXCLUSIVE` would be held only for that
   one table's copy. It *would* be the explanation for wrapping the whole
   load -- a single transaction over ~250 tables holds `ACCESS EXCLUSIVE` on
   all of them until it commits, blocking every reader for the length of the
   run.

   Three ways forward, cheapest first:

   - **Per-table atomic**: add `-v ON_ERROR_STOP=1` and restore the
     `begin`/`commit`. Readers of one table block for that table's copy
     (seconds); a failure leaves the old data in place. Two-line change.
   - **Whole-load atomic**: `psql --single-transaction -f script`. Correct,
     but blocks the site for the whole reload. Not worth it.
   - **Shadow schema + rename**: load into `<schema>_new`, then
     `ALTER SCHEMA ... RENAME` in one short transaction. Atomic across the
     whole database, locks measured in milliseconds. This is also what item 3
     wants, so the two are worth doing together.

3. **Rebuild the serving schema from the dictionary on reload** (owner's
   request, longer term). Today `schema.sql` is dumped but the DAG never
   passes `-c/--create`, so the serving tables must already exist and a
   dictionary change has to be applied there by hand. The schema is *derived*
   from the dictionary -- `loader/starschema.py` already generates it from
   `dict.adit_item_tbl` -- so the reload could create the tables it is about
   to fill. Doing that into a shadow schema and renaming gives correctness and
   atomicity in one step, and removes the "apply the DDL by hand" footgun.
   Note `load_postgres_db.py` runs under the system python3 with no psycopg2,
   so it would need the DDL handed to it (the dumped `schema.sql`, or one
   generated by `starschema` at dump time) rather than importing `loader`.
