-- The derived tables the BMRB-API queries: query_grid, chem_shifts,
-- instant_cache, instant_extra_search_terms, metabolomics_summary.
--
-- This was BMRB-API's server/wsgi/bmrbapi/reloaders/sql/initialize.sql, run as
-- a separate condor job after the reload had finished. It is here because it
-- has to be inside the swap to be correct: everything below builds on top of
-- `macromolecules`, `metabolomics` and `web`, all three of which the loader
-- now rebuilds as <schema>_new and renames into place in one transaction. Run
-- afterwards, against the live schemas, it would spend its whole run rebuilding
-- objects the swap had just destroyed -- the website losing the query grid and
-- instant search for the duration.
--
-- Run by loader/webextras.py as the last step of the web stage, with the
-- schema names rewritten to the shadows (loader/shadow.py) exactly as
-- webschema.sql and cs_stats.sql already are. Nothing here should be
-- schema-unqualified: the rewrite is what points it at the shadow, and an
-- unqualified reference would resolve against search_path and silently read
-- the live copy.
--
-- The _tmp-and-rename dance below is now belt and braces -- it builds into a
-- shadow schema nobody can read yet -- but it is harmless and left alone.
--
-- pg_trgm (needed for the gin_trgm_ops indexes below) is NOT created here.
-- webextras.ensure_extensions() installs it, into `public`, before the web
-- schema is built -- because an extension belongs to a schema, and an
-- unqualified CREATE EXTENSION lands it wherever search_path happens to point.
-- Landing in `web` would mean the first swap dropped it, and every
-- gin_trgm_ops index with it. That function also moves it out of the way if a
-- previous install put it somewhere the swap replaces.
--
-- Needs postgresql-contrib installed on the server.
